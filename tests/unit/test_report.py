...
from skillcraft.lint.report import format_github, format_sarif
...
class TestSarif:
    ...
    def test_windows_path_normalized_to_posix(self):
        uri = json.loads(format_sarif([_diag(file=r"src\skillcraft\rules.py", line=10)]))["runs"][
            0
...
class TestGithub:
    def test_windows_path_normalized_to_posix(self):
        # Regression: format_github shared the backslash bug with format_sarif;
        # the file= attribute must be a POSIX, repo-relative URI on every host.
        out = format_github([_diag(file=r"src\skillcraft\rules.py", line=10)])
        assert "file=src/skillcraft/rules.py" in out
        assert "\\" not in out
