...
def _under_folders(path: Path, parent: str, child: str) -> bool:
    ...
    parts = path.parts
    return any(
        parts[i] == parent and parts[i + 1] == child for i in range(len(parts) - 1)
    )
    ...
    def parse(self, path: Path, text: str) -> ConfigDoc:
        ...
        if has_fm:
            ...
            if parsed is None:
                ...
                frontmatter = parsed
                meta.description = parsed.get("description")
                meta.scope_globs = _as_glob_list(parsed.get("globs"))
                for key, val in parsed.items():
    ...
    def parse(self, path: Path, text: str) -> ConfigDoc:
        ...
        if has_fm:
            ...
            if parsed is None:
                ...
                frontmatter = parsed
                meta.description = parsed.get("description")
                meta.scope_globs = _as_glob_list(parsed.get("applyTo"))
                for key, val in parsed.items():
